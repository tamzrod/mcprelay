...
| S5 | The connector's own OAuth client credentials are exposed (client secret in code/config). | 1 | 3 | Store client secret in the secrets backend; PKCE reduces reliance on a static secret for public-client flows; never commit. |
...
| S7 | Open redirect / CSRF in the connector-hosted OAuth callback. | 1 | 3 | Validate `redirect_uri` against an allowlist; use `state` parameter; bind callback to the operator session. |
